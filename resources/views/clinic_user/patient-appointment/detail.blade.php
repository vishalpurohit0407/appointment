							...
							@else
								<div class="col-lg-12">
									<div class="font-weight-bold text-dark-50 py-2 py-lg-">
										<div class="d-flex align-items-center">
											<span class="text-dark-75 font-weight-bolder mr-4">RT PCR Status :</span>
											<input class="rt_pcr_status" data-switch="true" name="rt_pcr_status" type="checkbox" checked="checked" data-on-text="Positive" data-handle-width="70" data-off-text="Negative" data-on-color="success" data-off-color="danger" value="1" />
											<button class="ml-5 btn btn-info btn-sm save-rtpcr">Save</button>
										</div>
									</div>
								</div>
							@endif
							...
							@else
								<div class="col-lg-12">
									<div class="font-weight-bold text-dark-50 py-2 py-lg-">
										<div class="d-flex align-items-center">
											<span class="text-dark-75 font-weight-bolder mr-4">Antigens Status :</span>
											<input class="antigens_status" data-switch="true" name="antigens_status" type="checkbox" checked="checked" data-on-text="Positive" data-handle-width="70" data-off-text="Negative" data-on-color="success" data-off-color="danger" value="1" />
											<input type="text" class="form-control antigens_count w-100px float-left ml-5" placeholder=" Count">
											<button class="ml-5 btn btn-info btn-sm save-antigens">Save</button>
										</div>
									</div>
								</div>
							@endif
...
<script type="text/javascript">
	...
	{
		var appointment_id  = "{{ isset($patientAppointment->id) ? $patientAppointment->id : ''}}";
		var patient_id      = "{{ isset($patientAppointment->id) ? $patientAppointment->patient_id : ''}}";
		var antigens_count   = $('.antigens_count').val();
		var rt_pcr_status   = $('.rt_pcr_status').is(':checked') ? '1' : '0';
		var antigens_status = $('.antigens_status').is(':checked') ? '1' : '0';

		if(type=="antigens" && antigens_count == '0' && antigens_status == '1')
		{
			toastr.clear();
			toastr.error('Please enter Antigen Count');
			return false;
		}

		$.ajax({
			url : "{{route('patient.save-report')}}",
			type: "POST",
			data : {
				"patient_id"     : patient_id,
				"appointment_id" : appointment_id,
				"type"           : type,
				"rt_pcr_status"  : rt_pcr_status,
				"antigens_status" : antigens_status,
				"antigens_count" : antigens_count,
				"_token"         : "{{ csrf_token() }}"
			},
			dataType: "json",
